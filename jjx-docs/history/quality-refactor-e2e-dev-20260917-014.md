...
## 二、链 1：来料检验（可分批 + 不良进台账）

1. 采购入库单执行来料检验提交（分批：同一入库行多次提交 → 同一检验批更正，不重复记账）。
2. 期望：质量管理 → 来料检验（检验批）出现 IQC 批；列表显示 批量 / 已检 / 待检；
   批量区间命中抽样方案时，报告里 AQL / AC / RE 有值。
3. 有不良时：不良台账出现 NCR 记录（数量、CR/MA/MI、原因），检验批"待处置"数量 > 0。
4. 处置：退货类只在台账登记（来料退货走原有隔离/退货流程）；不良不得出现在良品入库数量里
   （允收入库上限 = 良品数，已在提交校验守卫）。

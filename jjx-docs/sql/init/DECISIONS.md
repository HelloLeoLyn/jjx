...
## 5 个待定项的拍板（2026-09-14 定，生产前置第 7 项）

1. **静态文件**（附件/图纸/打印模板）：随初始化数据一起交付，形态＝**tar 包 + 放置路径说明**（`JJX_UPLOAD_ROOT` 指到解包目录）；不进 git。
2. **目标库形状**：**仅数据**导入（结构由 DDL 负责）。DDL 基座暂缓期间用"现库结构基线"过渡；DDL 解封后改为"DDL 基线 + 数据子集"。
3. **ID 是否原样保留**：**原样保留**（业务单据互相引用，重编号会断链）。
4. **测试脏数据是否先清**：**不清**——生产初始化走"子集快照"，快照本身已剔除测试数据与日志/看板表；清理脚本只用于 dev。
5. **`quality_template_registry.file_id`**：**置空**（19 条悬挂引用；模板文件随静态文件包交付，迁移后可按文件名重新挂接）。

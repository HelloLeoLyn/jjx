...
**两个已知误报**（2026-09-14 记录，修复任务 dev-20260914-012，看到别慌）：
- 报「库中未记录已应用版本」→ 假警报：脚本抓的字段名和 `db-migrate.sh` 实际输出对不上；库里其实有记录（`ops.schema.applied` = 66…101,107,108,109）
- 结尾报「自检未通过」但上面没有 ✘ → 那只是"有未提交文件"的提醒，不是环境坏了

...
- **服务生命周期必须逐次明示授权**：除非用户在当前请求中明确要求启动、停止、重启、重载、杀进程或替换运行实例，否则任何 agent 都不得执行此类操作。实施、构建、测试、部署准备、E2E 验证等请求均不隐含服务操作授权；完成后只能报告“需要重启”，由用户操作。该规则覆盖后端/前端开发服务、OCR、systemd、容器、后台进程、端口占用进程及同类运行态操作。健康检查、端口/进程查看、日志读取等只读诊断可按任务需要执行，但不得改变运行状态。

教训存档（2026-09-10）：Hermes 在与用户讨论运维方案期间，连续落了 2 个文档并改了 INDEX.md，被用户叫停并立此条。

### 9.1 提案不得静默覆盖用户既有规矩（2026-09-23 立）

- agent 自拟方案只是提案；即使用户同意“一起改”，也不得据此默认废止更早的明确用户规则。
- 修改 `CONVENTIONS.md` 等规范真源前，必须查该规则的提交历史和用户原始口径；发现冲突时，先明确写出“这会改变用户既有规则 X”，取得针对该冲突的确认后再改。
- 影响面较大的口径变更，提案必须列出被替换的旧规则、迁移影响与回退方式，不能只描述新方案的优点。

教训存档（2026-09-23）：备份目录提案曾把用户既有“默认仓内、全库仅排除 `hr_employee`”静默改为仓外默认，后由 `cefa35fc` 恢复。提案获准不等于旧规矩自动失效。
...
## 14. 数据建模评审：新表 vs 配置 / 字典 / 属性（2026-09-24 立；起因：抽样方案 `quality_sampling_plan` 只有 8 行的讨论）

**一句话：新建表之前，先回答「这份数据能不能用 系统配置 / 字典 / 属性 装」——能装就别建表。**

**判据（默认优先走配置/字典/属性）**
- 行数少（经验阈值 **< 20 行**）、结构简单（键值对或单值）、基本不变、只给人看/选 → 走 `sys_config`（系统参数）/ `sys_dict`（字典）/ 现有表的 JSON 属性列
- 典型：状态枚举、单据类型、检验水平、费率/比例/开关类参数

**仍应建表的例外（两类）**
1. 一行含**多个数值字段**且有**区间/范围语义**（例：批量区间 → 抽检数 / Ac / Re），或需要**库级校验**（区间不重叠、允收<拒收等）；
2. 会被**业务单据引用**（外键、会落进业务数据），或需要**独立权限点 + 独立管理页 + 变更留痕**。

**硬压成配置时的已知代价（选了就要知情并记录）**：失去类型与 SQL 查询能力（只能在 Java/Vue 解析 JSON）、失去库级校验、多维度扩展（按客户/产品/严格度分档）会撑不住。

**配套要求**
- 配置/字典类数据**必须随迁移脚本**进新环境（写成幂等 INSERT 迁移），**不放**初始化数据清单（清单只放"从生产库导出的正式业务数据"）；
- 用 JSON 作配置值时注意列宽（`sys_config.config_value` 现为 `varchar(2000)`，超长会写入失败）。

**首个落地案例**：`quality_sampling_plan`（8 行、无迁移来源）→ 改为 `sys_config.quality.sampling_plan`（JSON），见 dev-20260924-021。

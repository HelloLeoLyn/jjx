<template>
  <div class="print-page">
    ...
    <A4Canvas v-if="info" :padding-mm="15">
      ...
      <table><thead><tr><th>序号</th><th>品名(料号)</th><th>规格</th><th>单位</th><th>数量</th><th>单价</th><th>金额</th><th>订单号码</th><th>备注</th></tr></thead><tbody><tr v-for="(item, index) in items" :key="item.id"><td class="center">{{ index + 1 }}</td><td>{{ item.productName || item.productCode || '-' }}</td><td>{{ item.specification || '-' }}</td><td>{{ item.unit || '-' }}</td><td class="right">{{ item.quantity ?? '-' }}</td><td class="right">{{ money(item.unitPrice) }}</td><td class="right">{{ money(item.amount) }}</td><td>{{ orderNo || '-' }}</td><td>{{ item.remark || item.lineRemark || '-' }}</td></tr><tr v-if="!items.length"><td colspan="9" class="center">无订单明细</td></tr></tbody></table>
...
const route = useRoute(), router = useRouter(), info = ref<SalesDeliveryVO>(), items = ref<any[]>([]), orderNo = ref('')
...
onMounted(async () => {
  if (!deliveryId) { ElMessage.error('发货单ID缺失'); return }

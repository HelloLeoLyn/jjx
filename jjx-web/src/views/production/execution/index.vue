...
const getList = async () => {
  loading.value = true
  try {
    const res: any = viewMode.value !== 'all'
      ? await getMyProductionExecutions(queryParams)
      : await getTaskTreePage({
          ...allQueryParams,
...
const switchView = (mode: 'mine' | 'all') => {
  ...
  else queryParams.pageNum = 1
  getList()
}
const loadAllTaskChildren = async (
  row: AllTaskRow,
  _treeNode: unknown,

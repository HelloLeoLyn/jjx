...
export const filmApi = {
  // 根据产品ID获取菲林列表
  getByProductId(productId: number): AxiosPromise<ProductFilm[]> {
    return request.get(`/engineering/films/product/${productId}`)
  },

  // 获取菲林详情
  getById(filmId: number): AxiosPromise<ProductFilm> {
    return request.get(`/engineering/films/${filmId}`)
  },

  // 创建菲林
  create(data: Partial<ProductFilm>): AxiosPromise<ProductFilm> {
    return request.post('/engineering/films', data)
  },

  // 更新菲林
  update(filmId: number, data: Partial<ProductFilm>): AxiosPromise<ProductFilm> {
  ...
  // 创建新版本
  newVersion(filmId: number): AxiosPromise<ProductFilm> {

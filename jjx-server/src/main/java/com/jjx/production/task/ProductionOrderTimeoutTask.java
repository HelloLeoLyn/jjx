            ...
            List<ProductionOrder> overdueOrders = productionOrderMapper.selectList(
                    new LambdaQueryWrapper<ProductionOrder>()
                            .in(ProductionOrder::getOrderStatus,
                                    ProductionOrderStatusEnum.PENDING_START.getValue(),
                                    ProductionOrderStatusEnum.IN_PROGRESS.getValue(),
                                    ProductionOrderStatusEnum.PAUSED.getValue())
                            .lt(ProductionOrder::getPlanEndDate, LocalDate.now()));
            int updated = 0;
            for (ProductionOrder order : overdueOrders) {
                order.setOrderStatus(ProductionOrderStatusEnum.OVERDUE.getValue());
